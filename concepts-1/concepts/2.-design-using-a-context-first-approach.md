---
description: >-
  Thinking in a context-first way is the single biggest step towards becoming a
  more effective OpenDialog conversation designer.
---

# 2. Design using a context-first approach

When thinking of how to design a conversational application we might be tempted to dive straight into the possible types of phrases that a user might say (the intents). It has, unfortunately, become common to discuss conversational applications in terms of the number of intents that they handle. We typically then jump straight into worrying about how to train a natural language understanding system (NLU) to identify those intents. Figuring out how to stitch those intents into a broader conversation becomes almost an afterthought.&#x20;
...
Along the way, we can define _**conditions**_ and _**behaviors** _and attach them to conversational components.&#x20;
