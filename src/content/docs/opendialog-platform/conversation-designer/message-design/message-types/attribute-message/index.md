...
`context_name` - name of a [context](/core-concepts/contexts-and-attributes/contexts) where to look for an attribute followed by dot `.` If context is omitted then conversation engine will assume `user` context.
