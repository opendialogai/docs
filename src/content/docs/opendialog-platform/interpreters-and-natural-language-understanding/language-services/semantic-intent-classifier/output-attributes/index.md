...
When using Semantic Intent Classifiers you may get other output attributes alongside any [custom output attributes](/opendialog-platform/interpreters-and-natural-language-understanding/language-services/semantic-intent-classifier#custom-output-attributes) you have defined. Please see the following sections for descriptions and examples of how you might use each of these attributes.
...
You could use this attribute in conditions in your conversation design. For example if you had a Semantic Intent Classifier to interpret a user utterance but it failed due to prompt moderation, you could use the `interpretation.flagged_moderation_categories` attribute to [condition](/core-concepts/contexts-and-attributes/conditions-and-operators) failure messages to let the user know what moderation category was flagged. These conditions would use the In Set operation, such as "interpretation.flagged\_moderation\_categories In Set violence" to condition a message to only display if the Semantic Intent Classifier failed due the violence category being flagged.
