...
![](~/assets/image-2.png "188")

![](~/assets/image-1-1.png "375")
...
*Example of basic configurations of webhook action*
...
* **URL**
  ...
  * It supports OD attribute syntax [like in messages](/opendialog-platform/conversation-designer/message-design/using-attributes-in-messages) with context and filters. You can use it to parametrise host name, path segment or query parameters. Resulting value is automatically URL encoded. Fore example:\
...
*Example of headers configuration of webhook action*
...
*Example of payload configuration for webhook action*
...
*Example of response configuration for webhook action*
...
For more complex mapping and response transformations, use [JMESPath](/opendialog-platform/actions/webhook-action/using-jmespath-expressions).
...
*Use testing panel on the left to run a test for your webhook*
...
![Authentication method selection showing Header, mTLS, and OAuth2 options](~/assets/authentication-selection.png)

*Select an authentication method for your webhook action*

:::note
All authentication methods are **secret-aware**. You can reference secrets from the [Secret Context](/core-concepts/contexts-and-attributes/secret-context) using `{secret.your_secret_name}` in your authentication configuration. Secret values are automatically decrypted during authentication and never appear in logs.
:::
...
![Header Authentication configuration](~/assets/authenticaton-headers.png)

*Header Authentication configuration*

#### Using secrets in headers

Header Authentication is secret-aware — you can reference secrets from the [Secret Context](/core-concepts/contexts-and-attributes/secret-context) in your header values and variables. When the authentication executes, secret values are automatically decrypted and included in the outgoing request.
...
![mTLS Authentication configuration](~/assets/authentication-mtls.png)
...
mTLS is secret-aware. The **Certificate** field expects a reference to a certificate stored in the [Secret Context](/core-concepts/contexts-and-attributes/secret-context). Certificates can be uploaded in P12, PFX, PEM, CER, CRT, or KEY formats. The **Password** field can also reference a secret, keeping your certificate passphrase protected.
...
:::note
To learn how to upload certificates and create secrets, see the [Secret Context](/core-concepts/contexts-and-attributes/secret-context) documentation.
:::
...
![OAuth2 Client Credentials configuration showing token URL, client ID, client secret with a secret reference, and scope](~/assets/oauth2-config.png)
