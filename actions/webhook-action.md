...
The webhook action allows you to send and receive data from an external service via a HTTP POST request to a provided webhook URL. For information on building such a service, please see our [developer documentation](https://docs.opendialog.ai/developing-with-opendialog/actions/webhook-action).
...
![A webhook action that sends/receives data from "https://api.example.com/opendialog"](<../.gitbook/assets/image (433).png>)

### Headers

You can add headers in the action configuration page that will be send along with the HTTP POST request.&#x20;

This could be used to send authorisation tokens amy other custom headers as required by the webhook.
